...
$(document).ready(function(){
    ...
    $('body').on('click','.cls-button-show',function(){
        winform(urlshow, {'id':$(this).data('id')}, 'Lihat hasil assignment');
    });
    $('body').on('click','.cancelAssignment',function(){
		onbtncancel(this);
	});

    $('.id_lembaga_assessment').select2({
        placeholder: "Pilih"
    });
...
function setDatatable(){
  ...
  .on('xhr.dt', function ( e, settings, json, xhr ) {
      ...
      columns: [
        ...
        { data: 'lembaga_assessment', orderable: false, className: "text-center", searchable: false},
        { data: 'assesment', orderable: false, className: "text-center", searchable: false},
        ...
        { data: 'cancel', className: "text-center", orderable: false, searchable: false},
      ],
...
function onbtncancel(element){
    swal.fire({
          title: "Pemberitahuan",
          text: "Yakin batalkan hasil Assessment ?",
          type: "warning",
          showCancelButton: true,
          ...
          if (result.value) {
              $.ajax({
                 url: urlcancelAssignment,
                 data:{id:$(element).data('id'),talenta_id:$(element).data('talenta_id')},
                 type:'post',
                 dataType:'json',
                 beforeSend: function(){
                  KTApp.block('.cls-content-data', {
  ...
  function changeLembaga(element) {
    swal.fire({
        title: "Pemberitahuan",
        ...
        if (result.value) {
            ...
            data:{
                ...
                assessment_id:$(element).data('assessment_id'),
            },
            type:'post',
            dataType:'json',
            beforeSend: function(){
