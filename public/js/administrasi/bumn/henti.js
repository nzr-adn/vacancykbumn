...
$(document).ready(function(){
	$('body').on('click','.cls-add-berhenti',function(){

		winform(urlcreatehenti, {'id_surat_keputusan':$('input[name=id_surat_keputusan]').val(), 'id_perusahaan':$('input[name=id_perusahaan]').val(), 'grup_jabatan_id':$('input[name=grup_jabatan_id]').val()}, 'Tambah Pemberhentian');
	});

	$('body').on('click','.cls-button-edit-henti',function(){
		winform(urledithenti, {'id':$(this).data('id'),'id_surat_keputusan':$('input[name=id_surat_keputusan]').val(), 'id_perusahaan':$('input[name=id_perusahaan]').val(), 'grup_jabatan_id':$('input[name=grup_jabatan_id]').val()}, 'Ubah Pemberhentian');
	});
...
function setDatatableHenti(){
  datatablehenti = $('#datatable-berhenti').on( 'preXhr.dt', function ( e, settings, processing ) {
  ...
  .on('xhr.dt', function ( e, settings, json, xhr ) {
      ...
      drawCallback: function( settings ) {
        var info = datatablehenti.page.info();
          $('[data-toggle="tooltip"]').tooltip();
          datatablehenti.column(0, {search:'applied', order:'applied'}).nodes().each( function (cell, i) {
...
function onbtndeletehenti(element){
	swal.fire({
        title: "Pemberitahuan",
        text: "Yakin hapus data Pemberhentian ?",
        type: "warning",
